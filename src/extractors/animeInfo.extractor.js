// src/extractors/animeInfo.extractor.js  (Deno-compatible)
...
async function extractAnimeInfo(id) {
  try {
    const [resp, characterData] = await Promise.all([
      ...
      axios.get(`https://${v1_base_url}/ajax/character/list/${id.split("-").pop()}`),
    ]);
    ...
    element.each((_, el) => {
      ...
      const value =
        key === "Genres" || key === "Producers"
          ? $(el).find("a").map((_, a) => $(a).text().split(" ").join("-").trim()).get()
    ...
    if (characterHtml) {
      charactersVoiceActors = $1(".bac-list-wrap .bac-item")
        .map((index, el) => {
          // same logic as original — keep as-is but using $1
          const character = {
            id:
              $1(el).find(".per-info.ltr .pi-avatar").attr("href")?.split("/")[2] || "",
